  ...
  - manager: apt-get   # Arbitrary name
    ...
    tasks:
      - task: sync-remotes  # Must match available routines, usually sync-|update-|install-
        ...
        privileged: true    # Run as super user, default is false
        interactive: false  # Allows prompting user in the terminal, or example for a super user password or confirmation
                            # If privilege escalation is required but interactive is set to false, then use pkexec or pinentry strategies
                            # Default is false
      - task: update-remotes
